...
session_start();
  ...
  $msg="" ;
 $i=1;
   $sql=mysqli_query($con, "SELECT id,studentname,class,type FROM examresult Where class=5 GROUP BY studentname");
   ...
   while($ROW=mysqli_fetch_array($sql))
    ...
    $TXTclass=$ROW['class'];
    $txttype=$ROW['type'];
    echo "<TR><TD>".$i++."</TD><TD>".$TXTs."</TD><TD>". $TXTclass."</TD><TD>".$txttype."</TD><TD colspan=3><a href=index.php?tag=viewfive&sn=".$ROW['studentname'].">VIEW</a></TD></TR>";
}
            }
             if($tag=="viewfive")
            {
